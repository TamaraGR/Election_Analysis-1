...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_list:
        ...
        print(f'{county} {county_percentage:.2f}% ({county_votes[county]:,d})')
         # 6e: Save the county votes to a text file.
        txt_file.write(f'{county} {county_percentage:.2f}% ({county_votes[county]:,d})\n')
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_votes[county] > largest_county):
            largest_county = county_votes[county]

    print()
    print("-------------------------")        
    for key, value in county_votes.items():
        if largest_county == value:
            print(f'Largest County Turnout: {key}')
            txt_file.write("\n")
            txt_file.write("-------------------------\n")
            txt_file.write(f'Largest County Turnout: {key}')
    txt_file.write("\n")
    txt_file.write("-------------------------\n")
    txt_file.write("\n")
